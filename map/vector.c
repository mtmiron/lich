...
void
vector_foreach(volatile vector_t *v, int (*func)())
{
	int i, ret;

	for (i = 0; i < v->num; i++)
	{
		ret = func((void *)v->data[i]);
...
/* returns the index of the passed value, or -1 if not found */
int
vector_find(vector_t *v, void *ptr)
{
	int i;

	for (i = 0; i < v->num; i++)
		if ((void *)ptr == (void *)v->data[i])
			return i;
